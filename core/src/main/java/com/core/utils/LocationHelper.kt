...
class LocationHelperImpl : LocationHelper, GoogleApiClient.ConnectionCallbacks,
    ...
    override fun locationPermissionRequired(): MutableLiveData<ArrayList<String>> =
        onPermissionRequest

    override fun requestStartUpdatingLocation(
        activity: WeakReference<Activity>,
        ...
        try {
            if (isPermissionGranted(activity) && isLocationEnable(
                    activity,
                    requestEnable != false
                )
            ) {
    ...
    private fun createLocationRequest(
        ...
        try {
            ...
            Looper.myLooper()?.let {
                fused.requestLocationUpdates(request, locationCallback, it)
            }
        } catch (e: Exception) {
            onStatus.postValue(LocationState.error(e.message))
        }
